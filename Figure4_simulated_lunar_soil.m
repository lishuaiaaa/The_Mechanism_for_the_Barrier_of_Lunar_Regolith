...
M=18;%水的分子量
t0=1e-13;%振动时间
Q=23600;%使用黄土的首层饱和吸附热值进行计算

r=(0.068*(10^-3))/2;%孔隙半径三维%%%体积中值孔径(Median Pore Diameter (Volume) )%%%
kongxilv=0.422778;%(rs-r1)/rs;压汞测出来
...
Dkthis2=(v*r*2)/(zuai);
% Dkthis3=(v*((kongxilv)^(2/3))*r*2)/(zuai);
Dkother=2/3*r*v;
...
dlmwrite('Msimulated_lunar_soil.txt',shuchu,'delimiter',',','-append','newline','pc','precision',10)
end
end
